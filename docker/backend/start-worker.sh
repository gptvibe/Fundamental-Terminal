...
if [ -n "${DATA_FETCHER_RUN_MACRO_WORKER+x}" ]; then
  RUN_MACRO_WORKER="${DATA_FETCHER_RUN_MACRO_WORKER}"
elif [ "${APP_PROFILE}" = "lite" ]; then
  RUN_MACRO_WORKER="false"
else
  RUN_MACRO_WORKER="true"
fi
INTERVAL="${WORKER_INTERVAL_SECONDS:-3600}"
if [ -n "${WORKER_IDENTIFIERS+x}" ]; then
  IDENTIFIERS_RAW="${WORKER_IDENTIFIERS}"
elif [ "${APP_PROFILE}" = "lite" ]; then
  IDENTIFIERS_RAW="AAPL,MSFT"
else
  IDENTIFIERS_RAW="AAPL MSFT NVDA"
fi
IDENTIFIERS="$(printf '%s' "${IDENTIFIERS_RAW}" | tr ',' ' ')"
