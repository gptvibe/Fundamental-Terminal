...
def test_oil_overlay_engine_applies_downstream_offset_for_integrated_names() -> None:
    payload = compute_oil_fair_value_overlay_payload(
        OilOverlayEngineInputs(
            base_fair_value_per_share=100.0,
            official_base_curve=(OilCurveYearPoint(year=2026, price=80.0),),
            ...
            diluted_shares=10.0,
            sensitivity_source_kind="manual_override",
            downstream_offset_percent=25.0,
            discount_assumptions=OilOverlayDiscountAssumptions(annual_discount_rate=0.0),
        )
    )

    assert payload["yearly_deltas"][0]["earnings_delta_after_tax"] == 75.0
    assert payload["overlay_pv_per_share"] == 7.5
    assert payload["assumptions"]["effective_downstream_offset_percent"] == 25.0


def test_oil_overlay_engine_ignores_downstream_offset_when_disclosed_sensitivity_is_used() -> None:
    payload = compute_oil_fair_value_overlay_payload(
        OilOverlayEngineInputs(
            base_fair_value_per_share=100.0,
            official_base_curve=(OilCurveYearPoint(year=2026, price=80.0),),
            ...
            annual_after_tax_oil_sensitivity=10.0,
            diluted_shares=10.0,
            sensitivity_source_kind="disclosed",
            downstream_offset_percent=40.0,
            discount_assumptions=OilOverlayDiscountAssumptions(annual_discount_rate=0.0),
        )
    )

    assert payload["yearly_deltas"][0]["earnings_delta_after_tax"] == 100.0
    assert payload["assumptions"]["effective_downstream_offset_percent"] == 0.0
