...
def test_oil_scenario_reads_cached_payload_without_live_fetch(monkeypatch):
    ...
    _patch_main_and_shared(monkeypatch, "get_company_oil_scenario_overlay_last_checked", lambda *_args, **_kwargs: datetime(2026, 4, 4, tzinfo=timezone.utc))
...
def test_oil_scenario_returns_stale_payload_and_queues_revalidation(monkeypatch):
    ...
    _patch_main_and_shared(monkeypatch, "get_company_oil_scenario_overlay_last_checked", lambda *_args, **_kwargs: datetime(2026, 4, 3, tzinfo=timezone.utc))
...
def test_oil_scenario_prefers_sec_disclosed_sensitivity_and_matching_benchmark(monkeypatch):
    ...
    _patch_main_and_shared(monkeypatch, "_resolve_cached_company_snapshot", lambda *_args, **_kwargs: _snapshot())
    _patch_main_and_shared(monkeypatch, "get_company_oil_scenario_overlay", lambda *_args, **_kwargs: (payload, "fresh"))
    _patch_main_and_shared(monkeypatch, "get_company_oil_scenario_overlay_last_checked", lambda *_args, **_kwargs: datetime(2026, 4, 4, tzinfo=timezone.utc))
    _patch_main_and_shared(monkeypatch, "queue_company_refresh", lambda *_args, **_kwargs: pytest.fail("refresh should not queue for fresh cache"))
    monkeypatch.setattr(
        oil_scenario_service,
