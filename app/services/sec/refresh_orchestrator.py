...
class EdgarIngestionService:
    ...
    def _load_refresh_bootstrap_inputs(
        ...
        sec_executor: ThreadPoolExecutor | None = None
        submissions_future: Future[dict[str, Any]] | None = None
        companyfacts_future: Future[dict[str, Any]] | None = None

        aux_io_fetch_budget = max(1, int(getattr(settings, "refresh_aux_io_max_workers", 1)))
        market_fetch_budget = max(1, int(getattr(settings, "market_max_concurrent_fetches", aux_io_fetch_budget)))
        if not settings.strict_official_mode and market_fetch_budget > 1:
            market_profile_executor = ThreadPoolExecutor(max_workers=market_fetch_budget, thread_name_prefix="refresh-market")
            market_profile_future = market_profile_executor.submit(self.market_data.get_market_profile, company_identity.ticker)

        sec_fetch_budget = max(1, int(getattr(settings, "sec_max_concurrent_fetches", aux_io_fetch_budget)))
        if sec_fetch_budget > 1:
            sec_executor = ThreadPoolExecutor(max_workers=sec_fetch_budget, thread_name_prefix="refresh-sec")
            submissions_future = sec_executor.submit(self.client.get_submissions, company_identity.cik)
            companyfacts_future = sec_executor.submit(self.client.get_companyfacts, company_identity.cik)

        try:
            submissions = submissions_future.result() if submissions_future is not None else self.client.get_submissions(company_identity.cik)
            ...
            companyfacts = companyfacts_future.result() if companyfacts_future is not None else self.client.get_companyfacts(company_identity.cik)
            ...
            if sec_executor is not None:
                sec_executor.shutdown(wait=True)
    ...
    def _refresh_from_sec(
        ...
        with SessionLocal() as session:
            ...
            if (
                ...
                existing_state = get_dataset_state(session, company.id, "prices")
                existing_payload_hash = existing_state.payload_version_hash if existing_state is not None else None
                price_prefetch_executor = ThreadPoolExecutor(
                    max_workers=max(1, int(getattr(settings, "market_max_concurrent_fetches", 1))),
                    thread_name_prefix="refresh-price",
                )
