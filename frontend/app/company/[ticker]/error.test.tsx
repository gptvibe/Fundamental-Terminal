...
describe("CompanyRouteError", () => {
  it("renders a research-brief fallback and retries the current page", () => {
    const reset = vi.fn();
    const consoleError = vi.spyOn(console, "error").mockImplementation(() => {});

    render(<CompanyRouteError error={new Error("boom")} reset={reset} />);

    expect(screen.getByRole("heading", { name: "Research Brief failed to load" })).toBeTruthy();
    expect(screen.getByText(/retry this brief/i)).toBeTruthy();
