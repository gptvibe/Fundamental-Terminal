...
import { buildOutlookChartShareSnapshot, buildStudioChartShareSnapshot, normalizeChartShareLayout } from "@/lib/chart-share";
      ...
      schedule_sections: [
        {
          ...
          rows: [
            { key: "revenue", label: "Revenue", unit: "usd", reported_values: { 2025: 100 }, projected_values: { 2026: 120 }, formula_traces: {}, scenario_values: {}, detail: null },
            { key: "operating_income", label: "Operating Income", unit: "usd", reported_values: {}, projected_values: { 2026: 24 }, formula_traces: {}, scenario_values: {}, detail: null },
            { key: "free_cash_flow", label: "Free Cash Flow", unit: "usd", reported_values: {}, projected_values: { 2026: 18 }, formula_traces: {}, scenario_values: {}, detail: null },
          ],
        },
      ],
    },
...
describe("chart share snapshot builders", () => {
  it("builds an outlook snapshot from the chart spec", () => {
    const snapshot = buildOutlookChartShareSnapshot(makePayload());

    expect(snapshot.mode).toBe("outlook");
    expect(snapshot.actual_label).toBe("Reported");
    expect(snapshot.forecast_label).toBe("Forecast");
    expect(snapshot.outlook?.primary_chart?.actual_points[0]?.label).toBe("FY2025");
  });

  it("builds a studio snapshot from the visible studio payload", () => {
    const snapshot = buildStudioChartShareSnapshot(makePayload(), { scenarioName: "Bull Case", overrideCount: 1 });

    expect(snapshot.mode).toBe("studio");
    expect(snapshot.studio?.scenario_name).toBe("Bull Case");
    expect(snapshot.studio?.forecast_year).toBe(2026);
    expect(snapshot.studio?.metrics[0]?.value).toBe("FY2026");
  });

  it("normalizes unsupported layouts back to landscape", () => {
    expect(normalizeChartShareLayout("banana")).toBe("landscape");
    expect(normalizeChartShareLayout("square")).toBe("square");
  });
});
